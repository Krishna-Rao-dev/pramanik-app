...
export default function Login({ onLogin }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.detail || "Invalid credentials. Try analyst@pramanik.in / password");
        setLoading(false);
        return;
      }
  ...
  return (
    <div style={s.page}>
      <div style={s.card}>
        ...
        <div style={s.hint}>
          <strong>Demo credentials:</strong> analyst@pramanik.in / password<br/>
          <span style={{ fontSize: 11, color: "var(--text-faint)", marginTop: 4, display: "block" }}>
            First run? POST /api/v1/auth/seed to create the demo user.
          </span>
        </div>
...
const styles = (theme) => ({
  ...
  logoIcon: { width: 36, height: 36, background: "var(--accent)", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 16, color: "#fff", flexShrink: 0 },
  logoName: { fontSize: 16, fontWeight: 700, color: "var(--text-primary)" },
  logoSub:  { fontSize: 11, color: "var(--text-muted)" },
  themeBtn: { marginLeft: "auto", background: "var(--bg-input)", border: "1px solid var(--border)", borderRadius: 6, padding: "4px 10px", fontSize: 14, cursor: "pointer" },
  ...
  submitBtn:  { width: "100%", padding: "11px", background: "var(--accent)", border: "none", borderRadius: 7, color: "#fff", fontWeight: 600, fontSize: 14, cursor: "pointer", marginBottom: 16 },
  hint:       { fontSize: 12, color: "var(--text-muted)", background: "var(--bg-input)", border: "1px solid var(--border)", borderRadius: 6, padding: "10px 12px", marginBottom: 20 },
