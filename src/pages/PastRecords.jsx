...
function RecordDetail({ record, onClose }) {
  ...
  const crossChk = MOCK.crossCheck;
      ...
      <div style={{ width: "65vw", maxWidth: 900, background: "var(--bg-base)", height: "100vh", overflowY: "auto", boxShadow: "-4px 0 24px rgba(0,0,0,0.15)" }}>
        ...
        <div style={{ padding: "18px 24px", background: "var(--bg-card)", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 10 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 2 }}>PAN: {record.pan} · Analysed: {record.date} · By: {record.analyst}</div>
          </div>
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <Badge color={record.complianceStatus === "PASS" ? "green" : "red"}>{record.complianceStatus}</Badge>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <Card noPad>
                ...
                <div style={{ padding: 16 }}>
                  <div style={{ marginBottom: 12 }}>
                    <div style={{ fontSize: 12, fontWeight: 600, color: "var(--success)", marginBottom: 8 }}>✓ Consistent Parameters ({crossChk.passed.length})</div>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                      {crossChk.passed.map(p => (
                  ...
                  {crossChk.failed.length > 0 && (
                    <div>
                      <div style={{ fontSize: 12, fontWeight: 600, color: "var(--danger)", marginBottom: 8 }}>✗ Anomalies Detected ({crossChk.failed.length})</div>
                      {crossChk.failed.map(f => (
                        <div key={f.parameter} style={{ padding: "10px 12px", background: "var(--danger-bg)", border: "1px solid var(--danger-border)", borderRadius: 6, marginBottom: 6 }}>
                          ...
                          <div style={{ fontSize: 12, color: "var(--text-muted)" }}>Expected: <span style={{ fontFamily: "var(--font-mono)", color: "var(--text-primary)" }}>{f.majority_value}</span> · Found: <span style={{ fontFamily: "var(--font-mono)", color: "var(--danger)" }}>{f.inconsistent_values.join(", ")}</span></div>
                        </div>
                      ))}
                    </div>
                  )}
              ...
              <Card noPad>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                  ...
                  <tbody>
                    {[
                      ["PAN",     "SANDBOX",     apiv.pan.status,     apiv.pan.name],
                      ["GST",     "GST PORTAL",  apiv.gst.status,     apiv.gst.legal_name],
                      ["LEI",     "GLEIF",       apiv.lei.status,     apiv.lei.corroboration],
                      ["CIN",     "MCA21",       apiv.cin.status,     apiv.cin.roc],
                      ["Pincode", "POSTAL API",  apiv.pincode.verified ? "Valid" : "Invalid", apiv.pincode.district],
                    ].map(([check, src, result, detail]) => (
                      <tr key={check} style={{ borderBottom: "1px solid var(--border)" }}>
                        ...
                        <td style={{ padding: "9px 14px" }}><span style={{ fontSize: 11, padding: "2px 7px", background: "var(--accent-light)", color: "var(--accent)", border: "1px solid var(--accent)", borderRadius: 4 }}>{src}</span></td>
                        <td style={{ padding: "9px 14px" }}><Badge color={["ACTIVE","Active","ISSUED","Valid"].includes(result) ? "green" : "red"}>{result}</Badge></td>
                        <td style={{ padding: "9px 14px", color: "var(--text-muted)", fontSize: 12 }}>{detail}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {tab === "fraud" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 12 }}>
                {[
                  { label: "Risk Score",     value: fraud.riskScore, unit: "/100",     color: "var(--success)"  },
                  { label: "Risk Level",     value: fraud.riskLevel, unit: "",         color: "var(--success)"  },
                  { label: "PEP Flags",      value: fraud.persons.filter(p=>p.pep).length,       unit: "",  color: "var(--text-primary)" },
                  { label: "RPT Entities",   value: fraud.rpt.length,                  unit: "",          color: "var(--warning)"  },
                ].map(m => (
                  <Card key={m.label} style={{ textAlign: "center", padding: "14px 12px" }}>
                    <div style={{ fontSize: 22, fontWeight: 700, color: m.color, fontFamily: "var(--font-mono)" }}>{m.value}<span style={{ fontSize: 12 }}>{m.unit}</span></div>
              ...
              {/* NLP Summary */}
              <Card noPad>
                <CardHeader title="AI Analysis Summary" subtitle="Generated by Qwen3 · Ollama" />
                <div style={{ padding: 16 }}>
...
export default function PastRecords() {
  ...
  const [results,  setResults]  = useState(MOCK.pastRecords);
  const [selected, setSelected] = useState(null);
  const [loading,  setLoading]  = useState(false);

  const handleSearch = () => {
    setLoading(true);
    // TODO: GET /api/v1/records/search?pan={search}
    setTimeout(() => {
      const filtered = MOCK.pastRecords.filter(r =>
        !search.trim() || r.pan.toLowerCase().includes(search.trim().toLowerCase()) ||
        r.company.toLowerCase().includes(search.trim().toLowerCase())
      );
      setResults(filtered);
      setLoading(false);
    }, 400);
  ...
  const columns = [
    ...
    { key: "complianceStatus",  label: "Compliance", render: v => <Badge color={v==="PASS"?"green":"red"}>{v}</Badge> },
    { key: "fraudRisk",         label: "Fraud Risk", render: v => <Badge color={v==="LOW"?"green":v==="MEDIUM"?"amber":"red"}>{v}</Badge> },
  ...
  return (
    <div style={{ padding: "28px 32px" }}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          <div style={{ flex: 1 }}>
            <input
              ...
              placeholder="Search by PAN number or company name (e.g. AAKCM1234C)"
              style={{ width: "100%", padding: "9px 12px" }}
            />
          </div>
          <Btn onClick={handleSearch} disabled={loading}>{loading ? "Searching..." : "Search"}</Btn>
          <Btn variant="ghost" onClick={() => { setSearch(""); setResults(MOCK.pastRecords); }}>Clear</Btn>
        </div>
        {search && <div style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 8 }}>{results.length} record(s) found</div>}
      </Card>
      ...
      <Card noPad>
        ...
        />
        <Table columns={columns} rows={results} emptyMsg="No records found for this PAN." />
      </Card>
